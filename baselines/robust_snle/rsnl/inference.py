...
def run_snl(
    ...
    target_accept_prob: Optional[float] = 0.8,
    num_rounds: int = 10,
    num_final_posterior_samples: int = 10_000,
    thinning: int = 10,
    num_warmup: int = 1000,
    num_chains: int = 4,
) -> MCMC:
    ...
    ----------
    model : Callable
        The target model for which the SNL sampler will be run.
    prior : dist.Distribution
        The prior distribution for the model parameters.
    sim_fn : Callable
        The DGP function given the model parameters.
    ...
    theta_dims : int, optional
        The number of dimensions in the parameter space. Defaults to 1.
    num_sims_per_round : int, optional
        Number of simulations per round. Defaults to 1000.
    target_accept_prob : float, optional
        Target acceptance probability for NUTS. Defaults to 0.8.
    num_rounds : int, optional
        Number of rounds of sequential inference. Defaults to 10.
    num_final_posterior_samples : int, optional
        Number of final posterior samples to collect. Defaults to 10_000.
    thinning : int, optional
        Thinning factor for MCMC samples. Defaults to 10.
