...
def main(config_name, beta = 100.0, pretrained = False, posterior_obtained = False, beta_tuning = False):
    ...
    # Setup paths
    project_root = Path(__file__).resolve().parent.parent.parent
    save_dir_inference = project_root / "baselines" / "results" / config_name / "neuralgbi" # save inference object here
    save_dir = project_root / "baselines" / "results" / config_name / "neuralgbi" /  f"beta_{int(beta)}"
    ...
    num_repeat = cfg.num_repeat if config_name != "turin" else 1
    ...
    for ind in range(num_repeat):
        ...
        # Convert x_obs to float32 tensor to match model dtype
        if isinstance(x_obs, np.ndarray):
            x_obs = torch.from_numpy(x_obs).float()
        elif isinstance(x_obs, torch.Tensor):
            x_obs = x_obs.float()
        else:
        ...
        if pretrained:
            ...
            # Load training data
            path_theta = training_data_dir / f"theta_{ind}_ace.pt"
            path_x_sim = training_data_dir / f"x_sim_{ind}_ace.pt"
