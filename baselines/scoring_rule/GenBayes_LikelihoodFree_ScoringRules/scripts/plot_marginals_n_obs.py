...
from src.utils import define_default_folders, extract_params_from_journal_gk, extract_params_from_journal_multiv_gk
from src.models import instantiate_model
import matplotlib.pyplot as plt
import matplotlib

parser = parser_plots_marginals_and_traces()
parser.add_argument('--appendix_plots', action="store_true", help="Create plot to put in Appendix for g-and-k.")
...
if model in ["univariate_g-and-k", "univariate_Cauchy_g-and-k"]:
    ...
    if model == "g-and-k":
        if appendix_plots:
            ...
            n_samples_in_obs_list_lists = [[1, 5, 10]] + [[1]]
        else:
...
savefig_name = inference_folder + f"Overall_plot_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_" \
                                  f"{n_samples_per_param}_thinning_{thin}_{cmap_name}"
if appendix_plots and model == "g-and-k":
    savefig_name += "_2"
plt.savefig(savefig_name + ".pdf")
