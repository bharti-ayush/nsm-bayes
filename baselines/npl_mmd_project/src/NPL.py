...
class npl():
    ...
    def __init__(self, X, B, m, p, l, model, Nstep = 1000, eta = 0.1, model_name = 'gaussian'):
        self.model = model
        self.B = B  
        self.m = m  
        self.p = p  
        ...
        if self.l == -1: # median heuristic
            # Original NPL formula: sqrt(0.5 * median(squared_distances))
            # Note: includes diagonal (which are 0) in median calculation
            self.l = np.sqrt((1/2)*np.median(distance.cdist(self.X,self.X,'sqeuclidean')))

            print(f"Lengthscale: {self.l}")
    ...
    def minimise_MMD(self, data, weights, batch_size=200):
        ...
        num_batches = self.n//batch_size # it is 1 in gandk
    ...
    def minimise_MMD_togswitch(self, data, weights, eta=0.04, batch_size=2000):
